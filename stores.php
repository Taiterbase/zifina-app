...
$sql = "SELECT * FROM " . $dbweb . ".store WHERE cat='1' ORDER BY id DESC";
$result = $db->query($sql);
...
?>
                <div class="col-lg-4 col-md-6 pb-4">
                    <div class="card dark" data-item-icon-name="<?php echo $item_icons; ?>" data-se="<?php echo $itemid; ?>" data-name="<?php echo $big_name; ?>" data-price="70" data-quality="<?php echo $item_color; ?>" data-itemid="<?php echo $itemid; ?>">
                        <div class="faction-icons">
                            <div class="game-icon--rounded game-icon game-icon-xs game-icon--faction-0">
                                <div class="game-icon-icon">
                                </div>
                            </div>
                            <div class="game-icon--rounded game-icon game-icon-xs game-icon--faction-1">
                                <div class="game-icon-icon">
                                </div>
                            </div>
                        </div>
                        <div class="product-image" style="background-image: url('<?php echo $item_image; ?>')"></div>
                        <div class="card-body">
                            <a href="https://tbcdb.com/?item=<?php echo $itemid; ?>" target="_blank" class="q-<?php echo $item_color; ?>-i"><?php echo $item_name; ?></a>
                            <div class="si-desc">100% Ground Mount</div>
...
<script src="/static/js/vendor/select2.min.js"></script>
	...
	if ($result->num_rows > 0) {
		...
		while($row2 = $result2->fetch_assoc()) {		
			...
			// if($row2["id"] == 0 && $row2["level"] < 58){
		
              // echo '<div class="alert alert-danger">Please go back and make sure you check the security CAPTCHA box.</div>';
 ...
 <script id="confirm-tmpl" type="text/x-handlebars-template">    
    ...
    <div class="row">
        <div class="col-md-12 text-center">
            ...
            <span class="text-warning">You need it level <strong>{{price}}</strong>.</span>
        </div>        
    </div>    
    </script>
